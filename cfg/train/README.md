...
顶层 `network_depth` 控制残差块数量（块数 = `network_depth // 4`）：

```bash
# 深度 16（4 个残差块）
uv run train --algo scaling_crl --task g1_box_placement --sim motrix network_depth=16

# 深度 64（16 个残差块）
uv run train --algo scaling_crl --task g1_box_placement --sim motrix network_depth=64
```
