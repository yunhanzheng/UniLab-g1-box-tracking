...
def run_mujoco_rollout_runner(
    ...
    sensor_buf = (
        np.empty((batch_size, 1, model.nsensordata), dtype=np.float64) if skip_checks else None
    )
...
def run_mjx(model, batch_size: int, steps: int, warmup: int = 5, device=None) -> SimRecord:
    ...
    try:
        ...
        if device:
            ...
            def warmup_run(d):
                d, _ = jax.lax.scan(
                    lambda dd, _: (batch_step_fn(mx_model, dd), None), d, None, length=10
                )
                return d
        else:
...
def plot_results(records: List[SimRecord], plot_dir: Path):
    ...
    ax.set_title(
        f"MuJoCo Simulation Speed (Steps Per Second) vs Batch Size\n{get_device_info_line()}"
    )
...
def main():
    ...
    parser.add_argument(
        "--out", type=str, default="benchmark/outputs/sim/results.json", help="Output JSON path"
    )
    parser.add_argument(
        "--plot-dir", type=str, default="benchmark/outputs/sim", help="Plot output directory"
    )
    parser.add_argument(
        "--motrixsim",
        type=str,
    ...
    for bs in batch_sizes:
        ...
        if bs <= 1024:
            try:
                ...
                print(
                    f"{r_cpu.backend:<25} | {bs:<6} | {r_cpu.sps:<12.1f} | {r_cpu.elapsed_sec:<8.4f}"
                )
        ...
        if bs <= 2048:
            try:
                r_roll1 = run_mujoco_rollout_runner(
                    model, bs, args.steps, 1, "mujoco_rollout_single"
                )
                print(
                    f"{r_roll1.backend:<25} | {bs:<6} | {r_roll1.sps:<12.1f} | {r_roll1.elapsed_sec:<8.4f}"
                )
        ...
        for skip_checks in skip_checks_flags:
            ...
            try:
                ...
                print(
                    f"{r_roll.backend:<25} | {bs:<6} | {r_roll.sps:<12.1f} | {r_roll.elapsed_sec:<8.4f}"
                )
        ...
        if run_warp_flag:
            try:
                ...
                print(
                    f"{r_warp.backend:<25} | {bs:<6} | {r_warp.sps:<12.1f} | {r_warp.elapsed_sec:<8.4f}"
                )
        ...
        if run_mjx_flag:
            if mjx and jax and jax.devices()[0].platform == "cpu":
                ...
                try:
                    ...
                    print(
                        f"{r_mjx.backend:<25} | {bs:<6} | {r_mjx.sps:<12.1f} | {r_mjx.elapsed_sec:<8.4f}"
                    )
                        ...
                        if bs <= 1024:
                            try:
                                ...
                                if not cpu_devs:
                                    ...
                                    print(
                                        f"{r_mjx_cpu.backend:<25} | {bs:<6} | {r_mjx_cpu.sps:<12.1f} | {r_mjx_cpu.elapsed_sec:<8.4f}"
                                    )
        ...
        if run_motrixsim_flag:
            try:
                ...
                print(
                    f"{r_mtx.backend:<25} | {bs:<6} | {r_mtx.sps:<12.1f} | {r_mtx.elapsed_sec:<8.4f}"
                )
